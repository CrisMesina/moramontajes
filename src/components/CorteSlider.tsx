...
interface CorteSliderProps {
    corte: corte[];
}

export const CorteSlider: React.FC<CorteSliderProps> = ({ corte }) => {
  const [currentIndex, setCurrentIndex] = useState(0);

  const goToPrevious = () => {
    const isFirstSlide = currentIndex === 0;
    const newIndex = isFirstSlide ? corte.length - 1 : currentIndex - 1;
    setCurrentIndex(newIndex);
  };

  const goToNext = () => {
    const isLastSlide = currentIndex === corte.length - 1;
  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto h-[500px]">
      ...
      <div
        ...
        style={{ backgroundImage: `url(${corte[currentIndex].imgURL})` }}
      >
      </div>
