...
interface BandaSliderProps {
    banda: banda[];
}

const BandaSlider: React.FC<BandaSliderProps> = ({ banda }) => {
  const [currentIndex, setCurrentIndex] = useState(0);

  const goToPrevious = () => {
    const isFirstSlide = currentIndex === 0;
    const newIndex = isFirstSlide ? banda.length - 1 : currentIndex - 1;
    setCurrentIndex(newIndex);
  };

  const goToNext = () => {
    const isLastSlide = currentIndex === banda.length - 1;
  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto h-[500px]">
        <div className='p-2 text-slate-500 font-bold text-center'>
            <span>Actualmente estas en: <a href="/">/</a><a href="/servicios/">Servicios/</a><a href="/servicios/transportadoras/">Banda</a></span>
        </div>
      <div
        className="w-full h-full rounded-xl bg-center bg-cover duration-500"
        style={{ backgroundImage: `url(${banda[currentIndex].imgURL})` }}
      >
      </div>
